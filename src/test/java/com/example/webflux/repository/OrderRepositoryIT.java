...
class OrderRepositoryIT {
    ...
    @Test
    @DisplayName("Save and find order by ID in PostgreSQL container")
    void saveAndFindOrderById() {
        Order order = new Order();
        order.setCustomerId("CUST-200");
        ...
        StepVerifier.create(savedAndRetrieved)
                .assertNext(found -> {
                    ...
                    assertThat(found.getCustomerId()).isEqualTo("CUST-200");
                    assertThat(found.getStatus()).isEqualTo(OrderStatus.NEW);
                    assertThat(found.getTotalAmount()).isEqualByComparingTo("250.00");
                    assertThat(found.getDiscountAmount()).isEqualByComparingTo("20.00");
                })
                .verifyComplete();
    }
}
